...
const askQuestions = async () => {
  ...
  const playerNames = await input({
    message: "Enter the enhanced players names (ex: Johnny;Jane): ",
  });
  const realmName = await input({
    message: "Enter name of the realm (ex: LivingFlame): ",
  });
...
const applyDmgModifier = (
  // receives all the player and raid lines to boost/subtract
  ...
  let { subtractor, remainder } = calculateSubtractorAndRemainder(
    rLines.length,
    totalDmgAdded
  );
...
const modifyDmg = (
  playerLines: Array<{ index: number; line: string }>,
  raidLines: Array<{ index: number; line: string }>,
  dmgModifier: number
) => {
  console.log(
    "player lines: " + playerLines.length + ". raid lines: " + raidLines.length
  );
  ...
  if (totalDmgAdded == 0) {
    ...
    console.log(
      "Something went wrong. Total damage added is " +
        totalDmgAdded +
        " but should be 0."
    );
  }
...
const readFileAndUpdateName = (path: string) => {
  file = readFileSync(path, "utf-8");
  let fileName = String(path.split("/").pop());
  const currentDate = new Date();
  fileName = fileName.replace(
    ".txt",
    "_modified" + currentDate.getTime() + ".txt"
  );
  return fileName;
};
...
async function main() {
  ...
  const playerLines = logParser.getPlayerDataByBoss();
  const raidLines = logParser.getRaidDataByBoss();

  modifyDmg(playerLines, raidLines, Number(answers.dmgModifier));
